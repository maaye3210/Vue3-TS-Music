<!-- 歌曲条目 -->
<template>
  <div class="flex song-item items-center w-full hover-bg-main px-2 rounded-sm" :class="{ 'playing': id === song.id }" @dblclick="play(song.id)">
    <div class="flex-shrink-0 flex-1 flex items-center justify-between pr-5 ">
      ...
      <div class="items-center flex flex-1 w-10 flex-shrink-0">
        <IconPark :icon="Like" size="16" :theme="isLove?'filled' : undefined" :class="isLove?'loved':'notlove'" />
        <div class="truncate" style="max-width: 75%;">
          <small>{{ song.name }}</small>
        </div>
        <IconPark v-if="song.mv > 0" class="ml-2 text-orange-400 cursor-pointer" size="16" :icon="PlayTwo" @click="router.push({ name: Pages.mvDetail, query: { id: song.mv } })" />
      </div>
