<template>
  <div v-if="showLyrics" class="absolute overflow-hidden z-50 w-full h-full">
    <div class="w-full h-full blur-lg bg-center" style="transform: scale(1.3)">
      <img :src="song.al?.picUrl" alt="" class="w-full">
    </div>
    <div class="absolute inset-0 bg-black bg-opacity-50  p-4 pb-0 flex flex-col">
      <!-- 返回 -->
      <div class="h-16 flex items-center">
        <icon-park v-on:click="changeShowLyrics" :icon="Down" class="text-gray-100 hover:text-teal-400" :size="32">
        </icon-park>
      </div>
      <!-- 歌词专辑部分 -->
      <div class="flex-grow flex justify-around items-center">
        ...
        <div class="flex-1 text-white flex flex-col justify-center text-center mr-10">
          <p class="text-xl my-4 flex justify-center leading-4">
            {{ song.name }}<icon-park v-on:click="changeShowLyrics" :icon="Youtube"
              class="text-gray-100 hover:text-teal-400 ml-2" :size="18"></icon-park>
          </p>
          ...
          <el-scrollbar max-height="24rem" ref="scrollbarRef">
            <div class="h-36"></div>
            <div v-for="(lyric, index) in lyriclist" class="my-2 text-sm h-6"
              :class="{ 'text-emerald-400 text-xl h-8': index === currentNode!.index }">
              {{ lyric }}
            </div>
            <div class="h-48"></div>
          </el-scrollbar>
...
watch(showLyrics, (newValue) => {
  ...
  if (newValue) {
    nextTick().then(
      ...
      () => (scrollbarContainer.value = scrollbarRef.value!.$el.getElementsByClassName('el-scrollbar__wrap')[0])
    )
  }
  // 隐藏操作
})
