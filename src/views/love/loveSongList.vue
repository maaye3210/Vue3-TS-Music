<template>
  ...
  <div class="grid grid-flow-row grid-cols-3 lg:grid-cols-5 gap-5 2xl:grid-cols-7">
    <div v-for="(item, index) in likeplaylists" :key="index" :class="{'item-1' : index===0}" @click="router.push({name:'playlist',query:{id:item.id}})">
...
const { likeplaylists }=toRefs(useUserLikeStore())
