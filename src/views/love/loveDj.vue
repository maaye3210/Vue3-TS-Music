...
<div class="grid grid-flow-row grid-cols-3 lg:grid-cols-5 gap-5 2xl:grid-cols-7">
  <div v-for="(item,index) in loveDj" :key="index" :class="{'item-1' : index===0}">
    <CoverPlay :name="item.name" :pic-url="item.picUrl" :play-count="item.programCount" show-play-count/>
...
import {userDjSublist} from '@/utils/api';
import {List,Search} from '@icon-park/vue-next'
import IconPark from "@/components/common/IconPark.vue";
import CoverPlay from '@/components//common//CoverPlay.vue'
import type {DjInfo} from '@/models/dj'

const router = useRouter();
const loveDj = ref<DjInfo[]>([])
onMounted(async()=>{
  loveDj.value = await userDjSublist()
})
